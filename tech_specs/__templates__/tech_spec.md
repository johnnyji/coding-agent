...
- Create a folder at `tech_specs/__agents__/<slug>/` (where `<slug>` is a kebab-case name derived from the feature, e.g. `bulk-csv-export`)
- Inside that folder, create two files:
  - `tech_spec.md` — the implementation spec, following the structure below
  - `qa_log.md` — seed this from `tech_specs/__templates__/qa_log.md` (copy the file contents as-is; the QA agent will append sessions to it)
- Reference back to this document in `tech_spec.md` so the AI agent knows how to implement against this style of tech spec
...
### Instructions (copy to every tech_spec.md as-is)

> IMPORTANT: If you are an implementor agent, first read `tech_specs/__templates__/tech_spec.md` and fully understand how to work with this tech spec before doing anything else.
...
- Order the sections chronologically based on dependency
...
5. After editing the spec file, review the work done in this section and determine if there are any QA steps a human or QA agent should verify. If yes, append a new QA entry to `qa_log.md` in the same folder using the format from `tech_specs/__templates__/qa_log.md`. The entry should be dated, reference the section name, and list the specific steps to verify the changes. If there are no relevant QA actions (e.g. the section was purely internal refactoring with no observable behavior change), skip this step.
...
Any questions that require user input that block the AI from continuing this section are recorded here. This will always be blank to start.

How AI agents interact with Blocking Questions:

1. As an AI agent comes across a blocking question in Asks, it will note the Ask item that is blocked, the progress it made so far, and write a description of why it's blocked and what input it needs from the user to continue.
