...
- [x] In `apps/api`, create `src/sandbox/SandboxManager.ts`. It must export a singleton class `SandboxManager` with the following public methods:
  ...
  **`async create(threadId: string, repoOwner: string, repoName: string, branch: string): Promise<SandboxInfo>`**
  ...
  2. Create a git worktree at `$SANDBOX_BASE_PATH/{threadId}/` from the mirror: `git worktree add --no-checkout <path> <branch>`. If the branch doesn't exist yet, create it from `develop`: `git worktree add -b <branch> <path> develop`.
  ...
  8. Run `mix ecto.migrate && mix run priv/repo/seeds.exs` inside the worktree (the database was already created in step 5; skip `ecto.create`).
...
- [x] Implement `src/graph/nodes/techSpec.ts`. This node must:
  ...
  3. Build a prompt instructing Claude Code to (all paths are relative to the cloned worktree at `state.sandboxPath` — never the orchestrator repo):
     ...
     - The spec's base branch for the PR should be `develop`.
